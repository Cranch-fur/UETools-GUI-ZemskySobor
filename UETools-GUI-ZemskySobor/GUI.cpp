...
void GUI::Draw()
{
	...
	if (GetIsMenuActive())
	{
		...
		{
			ImGui::Text("UETools GUI (Zemsky Sobor)");
			if (ImGui::IsItemHovered())
			{
			...
			SDK::UQuestManager* questManager = SDK::UQuestSubsystem::GetQuestManager(world);
			ImGui::BeginDisabled(questManager == nullptr);
			if (ImGui::BeginMenu("Zemsky Sobor - Quests"))
			{
				if (questManager)
				{
					ImGui::SetFontTitle();
					ImGui::Text("Quests");
					ImGui::SetFontRegular();
					if (ImGui::Button("Update##SmutaQuests"))
					{
						Features::SmutaQuests::Update();
						PlayActionSound(true);
					}
					ImGui::SameLine();
					ImGui::Spacing();
					ImGui::SameLine();
					ImGui::InputText("Search Filter##SmutaQuests", Features::SmutaQuests::filterBuffer, Features::SmutaQuests::filterBufferSize);
					ImGui::SameLine();
					ImGui::Spacing();
					ImGui::SameLine();
					ImGui::Checkbox("Case Sensitive##SmutaQuests", &Features::SmutaQuests::filterCaseSensitive);
					ImGui::SameLine();
					static int32_t smutaQuestKind = 0;
					ImGui::SetNextItemWidth(100);
					ImGui::SliderInt("Quest Kind", &smutaQuestKind, 0, 2);
					ImGui::SameLine();
					ImGui::TextHint("0 - Active\n1 - Finished\n2 - Tracking");

					std::vector<Smuta::QuestState::DataStructure> filteredSmutaQuests;
					switch (smutaQuestKind)
					{
						case 0:
							filteredSmutaQuests = Smuta::QuestState::FilterByObjectName(Features::SmutaQuests::activeQuests, Features::SmutaQuests::filterBuffer, Features::SmutaQuests::filterCaseSensitive);
							break;

						case 1:
							filteredSmutaQuests = Smuta::QuestState::FilterByObjectName(Features::SmutaQuests::finishedQuests, Features::SmutaQuests::filterBuffer, Features::SmutaQuests::filterCaseSensitive);
							break;

						case 2:
							filteredSmutaQuests = Smuta::QuestState::FilterByObjectName(Features::SmutaQuests::trackingQuests, Features::SmutaQuests::filterBuffer, Features::SmutaQuests::filterCaseSensitive);
							break;
					}

					for (Smuta::QuestState::DataStructure& questState : filteredSmutaQuests) // <-- Reference!
					{
						if (ImGui::TreeNode(questState.objectName.c_str()))
						{
							if (ImGui::Button("Start Quest"))
							{
								questManager->StartQuest(questState.reference->GetQuestAsset());
								PlayActionSound(true);
							}

							if (ImGui::Button("Track Quest"))
							{
								questManager->TrackQuest(questState.reference->GetQuestAsset());
								PlayActionSound(true);
							}
							ImGui::SameLine();
							if (ImGui::Button("Untrack Quest"))
							{
								questManager->UntrackQuest(questState.reference->GetQuestAsset());
								PlayActionSound(true);
							}

							if (ImGui::Button("Finish Quest (Succeeded)"))
							{
								questManager->FinishQuest(questState.reference->GetQuestAsset(), SDK::EQuestResult::Succeeded);
								PlayActionSound(true);
							}
							ImGui::SameLine();
							if (ImGui::Button("Finish Quest (Failed)"))
							{
								questManager->FinishQuest(questState.reference->GetQuestAsset(), SDK::EQuestResult::Failed);
								PlayActionSound(true);
							}
							ImGui::SameLine();
							if (ImGui::Button("Finish Quest (Restart)"))
							{
								questManager->FinishQuest(questState.reference->GetQuestAsset(), SDK::EQuestResult::Restart);
								PlayActionSound(true);
							}
					...
					ImGui::Text("Quest Manager Doesn't Exist!");
			...
			if (ImGui::BeginMenu("Zemsky Sobor - Levels"))
			{
				if (world)
				{
					if (ImGui::Button("TestLevel_Elagin"))
					{
						SDK::UKismetSystemLibrary::ExecuteConsoleCommand(world, L"travel TestLevel_Elagin", nullptr);
						PlayActionSound(true);
					}
					ImGui::SameLine();
					if (ImGui::Button("TestLevel_Surovcev"))
					{
						SDK::UKismetSystemLibrary::ExecuteConsoleCommand(world, L"travel TestLevel_Surovcev", nullptr);
						PlayActionSound(true);
					}

					if (ImGui::Button("Meadow_DarkForest"))
					{
						SDK::UKismetSystemLibrary::ExecuteConsoleCommand(world, L"travel Meadow_DarkForest", nullptr);
						PlayActionSound(true);
					}

					if (ImGui::Button("Lvl_Intro_DLC"))
					{
						SDK::UKismetSystemLibrary::ExecuteConsoleCommand(world, L"travel Lvl_Intro_DLC", nullptr);
						PlayActionSound(true);
					}
					ImGui::SameLine();
					if (ImGui::Button("Lvl_Pereulki_DLC_P"))
					{
						SDK::UKismetSystemLibrary::ExecuteConsoleCommand(world, L"travel Lvl_Pereulki_DLC_P", nullptr);
						PlayActionSound(true);
					}
...
bool Features::ActorTrace::Trace()
{
	...
	if (traceHit)
	{
		...
		SDK::UObject* hitObject = hitResult.HitObjectHandle.Actor.Get();
		if (hitObject)
		{
...
void Features::SmutaQuests::Update()
{
	SDK::UWorld* world = Unreal::World::Get();
	if (world == nullptr)
		return;
	...
	Features::SmutaQuests::activeQuests.clear();
	for (SDK::UQuestState* questState : questManager->ActiveQuests)
	{
		if (questState == nullptr)
			continue;

		Smuta::QuestState::DataStructure questStateData = {};

		questStateData.reference = questState;
		questStateData.className = questState->Class->GetFullName();
		questStateData.objectName = questState->GetFullName();

		Features::SmutaQuests::activeQuests.push_back(questStateData);
	}

	Features::SmutaQuests::finishedQuests.clear();
	for (SDK::UQuestState* questState : questManager->FinishedQuests)
	{
		if (questState == nullptr)
			continue;
		...
		Features::SmutaQuests::finishedQuests.push_back(questStateData);
	}

	Features::SmutaQuests::trackingQuests.clear();
	for (SDK::UQuestState* questState : questManager->TrackingQuests)
	{
		if (questState == nullptr)
			continue;
		...
		Features::SmutaQuests::trackingQuests.push_back(questStateData);
	}
}
