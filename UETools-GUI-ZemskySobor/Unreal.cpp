...
std::vector<Smuta::QuestState::DataStructure> Smuta::QuestState::FilterByObjectName(const std::vector<QuestState::DataStructure>& questStatesArray, const std::string& filter, const bool& caseSensitive)
{
	std::vector<QuestState::DataStructure> outCollection;
	size_t filterLength = filter.length();

	/* Filter Quests States by "Search Filter" */
	for (QuestState::DataStructure questState : questStatesArray)
	{
		/* "Search Filter" is empty - Quest State considered a match automatically. */
		bool matchFilters = filterLength == 0;

		if (matchFilters == false)
		{
			if (caseSensitive)
				matchFilters = questState.objectName.find(filter) != std::string::npos;
			else
			{
				std::string classNameLowerCase = questState.objectName;
				std::string filterLowerCase = filter;
		...
		if (matchFilters)
			outCollection.push_back(questState); // Quest State is good to go, can be considered "filtered".
	}
