...
namespace Smuta
{
	...
	public:
		struct DataStructure
		{
			SDK::UQuestState* reference;
			std::string className;
			std::string objectName;
		};


		static std::vector<QuestState::DataStructure> FilterByObjectName(const std::vector<QuestState::DataStructure>& questStatesArray, const std::string& filter, const bool& caseSensitive);
	};
}
